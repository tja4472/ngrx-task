...
export type MutableOnReducer<S, C extends ActionCreator[], D = Draft<S>> = (
  state: D,
  action: ActionType<C[number]>
) => void;

export function mutableOn<C1 extends ActionCreator, S>(
  creator1: C1,
...
export function mutableOn<
  ...
  S
>(creator1: C1, creator2: C2, reducer: MutableOnReducer<S, [C1, C2]>): On<S>;
export function mutableOn<
  C1 extends ActionCreator,
  C2 extends ActionCreator,
...
export function mutableOn<S>(
  ...
  const types = args.reduce(
    (result, creator): string[] => [...result, (creator as ActionCreator).type],
    [] as string[]
  );

  const mutableReducer = function reduce(state: S, action: Action): S {
    const producedState = produce(
      state,
      (draft: Draft<S>): Draft<S> => reducer(draft, action)
    );
